...
$deliveryId=(int)($input['delivery_id']??0); $items=$input['items']??[]; $reason=trim((string)($input['reason']??''));
...
try {
 ...
 $loc=$pdo->prepare("SELECT id FROM stock_locations WHERE sales_id=? AND location_type='SALES' AND is_active=1 LIMIT 1"); $loc->execute([$d['sales_id']]); $salesLoc=$loc->fetchColumn();
 if(!$salesLoc) throw new RuntimeException('Stock location Sales tidak ditemukan.',422);
 ...
 $ins->execute([$num,$d['id'],$d['sales_id'],$d['outlet_id'],$reason?:null,$user['id']]); $returnId=(int)$pdo->lastInsertId();
 $ri=$pdo->prepare('INSERT INTO return_items(return_id,product_id,qty) VALUES(?,?,?)');
 foreach($items as $item){
   $pid=(int)($item['product_id']??0); $qty=(float)($item['qty']??0); if($pid<1||$qty<=0) throw new RuntimeException('Item retur tidak valid.',422);
   $del=$pdo->prepare('SELECT qty FROM delivery_items WHERE delivery_id=? AND product_id=? LIMIT 1'); $del->execute([$deliveryId,$pid]); $delivered=(float)$del->fetchColumn();
   if($delivered<=0 || $qty>$delivered) throw new RuntimeException('Qty retur melebihi qty delivery.',409);
